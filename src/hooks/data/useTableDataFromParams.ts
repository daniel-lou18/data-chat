...
export function useTableDataFromParams() {
  ...
  const communesQuery = useGetAggregatesByInseeCode<CommuneTableData[]>(
  ...
  const sectionsQuery = useGetAggregatesByInseeCodeAndSection<
    SectionTableData[]
  >(
  ...
  return {
    activeQuery:
      filterState.level === "section" ? sectionsQuery : communesQuery,
    communesQuery,
    sectionsQuery,
  };
}
